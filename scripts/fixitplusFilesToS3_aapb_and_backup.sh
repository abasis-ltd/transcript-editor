#!/bin/bash
# created for WGBH Media Library and Archives
# by Kevin Carter, 2023-11-02
# modified 2024-02-27 to handle backup copies 

export endpointURL='https://s3.us-east-1.amazonaws.com' ;
export bucketName='americanarchive.org' ;

export profileName=''   # personalize this for your own use to avoid subsequent dialogs/prompts for the profile;
...
arg=$1;
    ...
    guid=$(echo "$fileName" | sed 's#^.*cpb-aacip.#cpb-aacip-#1;s#[^[:alnum:]-].*##1;s#\(.*\)-transcript#\1#ig' );
...
#     fi
    if [ "$fileName" != "$guid"'-transcript.json' ]; then
      errString=$(echo "$errString '$arg' file name should be '$guid-transcript.json' for use by the AAPB site");
    fi
    if [ -z "$errString" ]; then 
      ...
      aws s3api copy-object --profile "$profileName" --bucket "$bucketName" --copy-source "$bucketName/transcripts/$guid/$fileName" --key "transcripts/ORIGINAL-""$guid"/"$guid""-transcript""$copyVersion"".json" --metadata-directive 'COPY' --tagging-directive 'COPY' >/dev/null ;
      echo "$guid/$fileName"
    else
...
#     echo "Line: ${line}"
      msg=$(awsCopyPush $line) ;
      echo "$msg" | grep -v '^cpb-aacip';
      key=$(echo "$msg" | tail -1 | grep '^cpb-aacip');
      if [ ! -z "$key" ]
      then
        aws s3api put-object --profile "$profileName" --bucket "$bucketName" --key transcripts/"$key" --body "$line" >/dev/null ;
        aws s3api put-object --profile "$profileName" --bucket "$bucketName" --key backup/transcripts/fixitplus/"$(basename $key)" --body "$line" >/dev/null ;
      fi
    done
...
#   echo "No input was found on stdin, skipping!"
  ...
  do
    ...
    if [ ! -z "$key" ]
    then
      aws s3api put-object --profile "$profileName" --bucket "$bucketName" --key transcripts/"$key" --body "$arg" >/dev/null ;
      aws s3api put-object --profile "$profileName" --bucket "$bucketName" --key backup/transcripts/fixitplus/"$(basename $key)" --body "$arg" >/dev/null ;
    fi
  done

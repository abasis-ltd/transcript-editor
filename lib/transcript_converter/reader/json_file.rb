    ...
    class JSONFile
      ...
      def parse_json
        if valid_json?(File.read(file))
          ...
          raise "#{File.basename(file.path)}: JSON is not in the expected format." unless json.has_key?('phrases')
          return { "file_name" => "#{File.basename(file.path, '.json')}", "phrases" => json["phrases"] }
        else
          nil
        end
      end
